...
| `public_base_url` | `str` | `""` | 可选，仅用于上传成功后显示链接的拼接前缀；当 `base_url` 使用内网地址时，可填写对外可访问的公网地址 |
| `upload_admin_only` | `bool` | `true` | 是否仅管理员可上传 |
| `auth_code` | `str` | `""` | 上传认证码（可选） |
| `show_upload_link` | `bool` | `true` | 上传成功时是否显示链接 |
| `local_random_type` | `bool` | `false` | 媒体类型本地随机。若开启且请求包含图片和视频，则在本地随机选取其中一种类型后再请求图床，用于平衡图片和视频出现概率。 |
| `keyword_recent_media_limit` | `int` | `0` | 关键词去重窗口。`0` 为禁用；`N>0` 表示每个关键词记录最近 N 次返回的媒体 ID。 |
| `keyword_dedupe_retry_limit` | `int` | `2` | 命中近期媒体 ID 时的重试次数。达到上限后，回退使用近期窗口中距离最远的 ID。 |
